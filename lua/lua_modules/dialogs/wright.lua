...
function wrightAntagConvReload()
...
--- Table Definitions
------------------------------------------------------------------------------
function wrightAntagConv(screenInstance)
    local response = {
        "Okay, I'll leave. <End conversation>",
    }
    local npcWords = {
    ...
    local function wrightAntagScreen1()
        scream.npcText = npcWords[1]
        scream.playerResponses = { response[1],}
        scream.playerActions = { actions[1],}
...
function wrightConv1Screen(screenInstance)
    local repMission = factions.republic.mission.medicalSupplies
    local unscMission = factions.unsc.mission.clearOut
    local response = {
    ...
    local function wright_supplyConIntro1_alt1()
        ...
        scream.playerActions = { actionsArray[2], actionsArray[10] }
        if unscMission.active then -- Conditional: Spoken to Forbes first (wrightSupplyCon_Intro1)
            scream.npcText = npcWords[1]
            ...
            --console_out("wright_supplyConIntro1_alt1") -- Intro if Wright is met before Forbes.
        end
    end
...
function wrightConv2Screen(screenInstance)
    --[[
    This conversation screen appears following Conversation 1. It will then change if the player's inventory contains "medicines" or "big medicines".
  
    for supplies = 1, playerInventory.length do
        if (playerInventory[supplies] == "medicines") then
          local repMission = factions.republic.mission.medicalSupplies
          repMission.resolved = true
            --console_out("success")
        end
    end]]
  local responses = {
    ...
    "I've got the medical supplies.", -- 3
  }

  local npcWords = {
    "Spartan, you're back. Have you managed to find any supplies to treat our wounded?", -- 1
    "Thank you Spartan. We'll get our team removed from the cave with no further incident. You've really helped our community.", -- 2
    "Thank you Spartan. I'll pass the word around. You've really helped our community. Thank you.", -- 3
    "Sorry Spartan, I don't have time for questions."
  }
  ...
  local scream = {}
  local function wrightConv2_intro()
      scream.npcText = npcWords[1]
      scream.playerResponses = {responses[1], responses[2], responses[3]}
      scream.playerActions = {actions[1], actions[1], actions[1]}
  end
...
function wrightConversations()
    local republic = factions.republic
    local unsc = factions.unsc
    local covenant = factions.covenant
    local wrightRelationship = republic.members.wright.relationship
    local wrightMet = republic.members.wright.met
    local functions = {
      dialog.open(wrightConv1Screen(get_global("conv_short1")), true),
      dialog.open(wrightAntagConv(get_global("conv_short1")), true),
    }
