...
function exampleConvReload()   -- IMPORTANT - give this function a new name for every conversation. In VSCode, highlight the name and press CTRL+F2 to rename all. If you don't, the conversations will interfere with each other and load conversations from other modules
    dialog.open(raiderGeneric(get_global("conv_short1")), true)            -- Reopens the conversation based on the global defined. You can change this.
    stop_timer(periodic)                                                                        -- stop timer defined in actions array as "periodic"
end

function raiderGeneric(screenInstance)
    ------------------------------------------------------------------------------
    --- Dialogue Arrays
    ------------------------------------------------------------------------------
    local response = {                                  -- Player                       
        "Forbes sent me to clear out these caves. What do you think?", ,-- 1
        "How are you boys doing?", -- 2
        }

    local npcWords = {                                  -- NPC
        "I think you can talk to the boss if you want us to leave, else you might be in this cave for the rest of your life.",  ,-- 
        "Howdy.",
    }  
    ------------------------------------------------------------------------------
    --- Actions Array 
